    ...
    public class MonthlyReportService : IMonthlyReportService
    {
        ...
        public async Task<(byte[] Content, string FileName)> GenerateMonthlyReportAsync(MonthlyReportDto dto)
        {
            ...
            var data = await _unitOfWork
                ...
                .Include(e => e.EppDetails)
                    .ThenInclude(d => d.EppType)
                .Include(e => e.EppDetails)
            ...
            if (!data.Any())
                throw new InvalidOperationException("No hay datos para el mes especificado");
        ...
        "Fecha solicitud"
            ...
            {
                foreach (var detail in item.EppDetails)
                {
                    worksheet.Cell(row, 1).Value = item.Name;
                    worksheet.Cell(row, 2).Value = item.Area;
                    worksheet.Cell(row, 3).Value = item.Position;

                    worksheet.Cell(row, 4).Value = detail.EppType?.NameType ?? "N/A";
                    worksheet.Cell(row, 5).Value = detail.Size?.NameSize ?? "No aplica";
                    worksheet.Cell(row, 6).Value = detail.RequestedQuantity;
